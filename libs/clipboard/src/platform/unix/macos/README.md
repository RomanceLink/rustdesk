...
1. Use a temporary file `/tmp/laladesk_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.laladesk_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.laladesk_*`.
