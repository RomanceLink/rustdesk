...
#define RUSTDESK_LIB_PATH "liblaladesk.so"
typedef bool (*LaLaDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_laladesk_core_main() {
   void* liblaladesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!liblaladesk) {
      fprintf(stderr,"Failed to load \"liblaladesk.so\"\n");
      char* error;
   ...
   auto core_main = (LaLaDeskCoreMain) dlsym(liblaladesk,"laladesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"laladesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
