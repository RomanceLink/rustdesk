...
g_arpsystemcomponent = {
    ...
    "Contact": {
        "msi": "ARPCONTACT",
        "v": "https://github.com/laladesk/laladesk",
    },
    "HelpLink": {
        "msi": "ARPHELPLINK",
        "v": "https://github.com/laladesk/laladesk/issues/",
    },
    "ReadMe": {
        "msi": "ARPREADME",
        "v": "https://github.com/laladesk/laladesk",
...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="LaLaDesk", help="The app name."
    )
    parser.add_argument(
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "Package/Components/LaLaDesk.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("LaLaDesk", app_name)
...
def replace_app_name_in_custom_actions(app_name):
    ...
    for file_path in chain(custion_actions_dir.glob("*.cpp"), custion_actions_dir.glob("*.h")):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            line = re.sub(r"\bLaLaDesk\b", app_name, line)
            line = line.replace(f"{app_name} v4 Printer Driver", "LaLaDesk v4 Printer Driver")
            lines[i] = line
        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(lines)
...
def update_license_file(app_name):
    if app_name == "LaLaDesk":
        return
    license_file = Path(sys.argv[0]).parent.joinpath("Package/License.rtf")
    with open(license_file, "r", encoding="utf-8") as f:
        license_content = f.read()
    license_content = license_content.replace("website laladesk.com and other ", "")
    license_content = license_content.replace("LaLaDesk", app_name)
