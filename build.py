...
hbb_name = 'laladesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: laladesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: laladesk <info@laladesk.com>
Homepage: https://laladesk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/laladesk')
    system2('mkdir -p tmpdeb/etc/laladesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/laladesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/laladesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/laladesk/')
    system2(
        'cp ../res/laladesk.service tmpdeb/usr/share/laladesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/laladesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/laladesk.svg')
    system2(
        'cp ../res/laladesk.desktop tmpdeb/usr/share/applications/laladesk.desktop')
    system2(
        'cp ../res/laladesk-link.desktop tmpdeb/usr/share/applications/laladesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/laladesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/laladesk/')
    system2(
        'cp ../res/pam.d/laladesk.debian tmpdeb/etc/pam.d/laladesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/laladesk/files/polkit && chmod a+x tmpdeb/usr/share/laladesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb laladesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('laladesk.deb', '../laladesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/laladesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/laladesk/')
    system2(
        'cp ../res/laladesk.service tmpdeb/usr/share/laladesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/laladesk.png')
...
def build_flutter_dmg(version, features):
    ...
    os.rename("laladesk.dmg", f"../laladesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/liblaladesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/liblaladesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/laladesk.exe')
    os.chdir('../..')
    if os.path.exists('./laladesk_portable.exe'):
        os.replace('./target/release/laladesk-portable-packer.exe',
                   './laladesk_portable.exe')
    else:
        os.rename('./target/release/laladesk-portable-packer.exe',
                  './laladesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/laladesk_portable.exe')
    os.rename('./laladesk_portable.exe', f'./laladesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/laladesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/laladesk.exe')
        system2('mv target/release/laladesk.exe target/release/LaLaDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\laladesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/LaLaDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/laladesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/laladesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/laladesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv laladesk-%s-0-x86_64.pkg.tar.zst laladesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./laladesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/laladesk-%s-0.x86_64.rpm ./laladesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall laladesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/laladesk-%s-0.x86_64.rpm ./laladesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall laladesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/laladesk*.deb ./flutter/laladesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/LaLaDesk.app/Contents/MacOS/laladesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/LaLaDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/laladesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LaLaDesk.app/Contents/MacOS/laladesk
    #rcodesign sign --p12-file ~/.p12/laladesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LaLaDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/laladesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LaLaDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LaLaDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LaLaDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "LaLaDesk %s.dmg" "target/release/bundle/osx/LaLaDesk.app"' % version)
                os.rename('LaLaDesk %s.dmg' %
                          version, 'laladesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/laladesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./laladesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./laladesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple laladesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/LaLaDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/laladesk*.deb ./laladesk.deb')
                system2('dpkg-deb -R laladesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/laladesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/laladesk-link.desktop tmpdeb/usr/share/applications/laladesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/laladesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/laladesk/')
                os.system('mkdir -p tmpdeb/etc/X11/laladesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/laladesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/laladesk.debian tmpdeb/etc/pam.d/laladesk')
                system2('strip tmpdeb/usr/bin/laladesk')
                system2('mkdir -p tmpdeb/usr/share/laladesk')
                system2('mv tmpdeb/usr/bin/laladesk tmpdeb/usr/share/laladesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/laladesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb laladesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('laladesk.deb', 'laladesk-%s.deb' % version)
