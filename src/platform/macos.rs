...
const UPDATE_TEMP_DIR: &str = "/tmp/.laladeskupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.laladesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.laladesk", &bundleid);
    }
    s = s.replace("laladesk", &crate::get_app_name().to_lowercase());
    s = s.replace("LaLaDesk", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/LaLaDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another LaLaDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // LaLaDesk.app/Contents/MacOS/LaLaDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'LaLaDesk' | grep -v {} | xargs kill -9 && rm -rf /Applications/LaLaDesk.app && ditto '{}' /Applications/LaLaDesk.app && chown -R {}:staff /Applications/LaLaDesk.app && xattr -r -d com.apple.quarantine /Applications/LaLaDesk.app
" with prompt "LaLaDesk wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/LaLaDeskUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/LaLaDesk.app/Contents/MacOS/LaLaDesk", target_dir);
    let _child = unsafe {
