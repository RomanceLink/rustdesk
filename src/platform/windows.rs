    ...
    loop {
        ...
        if !sids.contains(&session_id) || !is_share_rdp() {
            ...
            if session_id != current_active_session {
                ...
                // https://github.com/laladesk/laladesk/discussions/10039
...
fn get_after_install(
    ...
    // https://github.com/laladesk/laladesk/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/laladesk/laladesk/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/laladesk/laladesk/issues/6786#issuecomment-1879655410
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging LaLaDesk.
...
mod cert {
    ...
    extern "C" {
        fn DeleteLaLaDeskTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeleteLaLaDeskTestCertsW();
        }
        Ok(())
    }
}
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `laladesk --connect` are running.
...
pub fn try_kill_laladesk_main_window_process() -> ResultType<()> {
    // Kill laladesk.exe without extra arg, should only be called by --server
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    log::info!("try kill laladesk main window process");
    ...
    bail!("failed to find laladesk main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_laladesk.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\LaLaDesk\LaLaDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\LaLaDesk\LaLaDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
