...
// stop_laladesk_servers() is just a temp solution here.
fn force_stop_server() {
    stop_laladesk_servers();
...
pub fn start_os_service() {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if keeps_headless || keeps_session {
            // for fixing https://github.com/laladesk/laladesk/issues/3129 to avoid too much dbus calling,
            sleep_millis(500);
        } else {
            sleep_millis(super::SERVICE_INTERVAL);
        }
...
pub fn is_locked() -> bool {
    ...
    // https://github.com/laladesk/hbb_common/blob/ebb4d4a48cf7ed6ca62e93f8ed124065c6408536/src/platform/linux.rs#L119
    if values.is_empty() {
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub xauth: String,
        pub home: String,
        pub is_laladesk_subprocess: bool,
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_laladesk_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_laladesk_subprocess = false;
            let cmd = format!(
            ...
            if let Ok(res) = run_cmds(&cmd) {
                if res.trim() != "0" {
                    self.is_laladesk_subprocess = true;
                }
            }
        }
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill laladesk --tray, execute cp first
...
pub fn check_autostart_config() -> ResultType<()> {
    ...
    // https://github.com/laladesk/laladesk/issues/4863
