// https://github.com/laladesk/laladesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteLaLaDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes LaLaDesk Test certificates and wrong key stores
//
...
BOOL DeleteLaLaDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteLaLaDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteLaLaDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteLaLaDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteLaLaDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
