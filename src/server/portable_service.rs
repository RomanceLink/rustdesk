...
pub mod server {
    ...
    async fn run_ipc_client() {
        ...
        match ipc::connect(1000, postfix).await {
            Ok(mut stream) => {
                let mut timer =
                    crate::laladesk_interval(tokio::time::interval(Duration::from_secs(1)));
                let mut nack = 0;
                loop {
                    tokio::select! {
                        res = stream.next() => {
                ...
                match hbb_common::directories_next::UserDirs::new() {
                    Some(user_dir) => {
                        let dir = user_dir
                            ...
                            .join("AppData")
                            .join("Local")
                            .join("laladesk-sciter");
                        if std::fs::create_dir_all(&dir).is_ok() {
                            let dst = dir.join("laladesk.exe");
                            ...
                            match result {
                                Ok(stream) => {
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        let mut timer = crate::laladesk_interval(tokio::time::interval(Duration::from_secs(1)));
                                        let mut nack = 0;
