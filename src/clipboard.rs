...
const RUSTDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.laladesk.owner";
...
pub fn is_file_url_set_by_laladesk(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.laladesk_", "//tmp/.laladesk_"] {
...
impl ClipboardContext {
    ...
    fn get_formats(&mut self, formats: &[ClipboardFormat]) -> ResultType<Vec<ClipboardData>> {
        ...
        // Related issues:
        // https://github.com/laladesk/laladesk/issues/9263
        // https://github.com/laladesk/laladesk/issues/9222#issuecomment-2329233175
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_laladesk(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_laladesk(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_laladesk(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_laladesk(data, side);
            if !urls.is_empty() {
                // FIXME:
                ...
                let clear_holder_text = if is_kde_x11 {
                    "LaLaDesk placeholder to clear the file clipbard"
                } else {
                    ""
                }
                .to_string();
