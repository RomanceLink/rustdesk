...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "laladesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/laladesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "LaLaDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "LaLaDesk wants to uninstall the plugin",
    )
}
