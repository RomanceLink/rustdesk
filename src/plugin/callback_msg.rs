...
const MSG_TO_RUSTDESK_TARGET: &str = "laladesk";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_RUSTDESK_TARGET => handle_msg_to_laladesk(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_laladesk(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_laladesk = early_return_value!(
        serde_json::from_str::<MsgToLaLaDesk>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_laladesk.r#type as &str {
        MSG_TO_RUSTDESK_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_laladesk),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
...
fn request_plugin_sign(id: String, msg_to_laladesk: MsgToLaLaDesk) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_laladesk.data),
        ERR_CALLBACK_INVALID_MSG,
