    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            return match crate::platform::windows::is_msi_installed() {
                Ok(true) => format!("laladesk-{_version}-x86_64.msi"),
                Ok(false) => format!("laladesk-{_version}-x86_64.exe"),
                Err(e) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("laladesk-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("laladesk-{_version}-aarch64.dmg")
                } else {
