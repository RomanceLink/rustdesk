...
async fn start_hbbs_sync_async() {
    let mut interval = crate::laladesk_interval(tokio::time::interval_at(
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                //
                // https://github.com/laladesk/laladesk/discussions/8031
...
fn heartbeat_url() -> String {
    ...
    if url.is_empty() || url.contains("laladesk.com") {
        return "".to_owned();
    }
